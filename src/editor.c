...
void *event_track_next(void *data) {
    api_t *api = (api_t *)data;
    list_active_next(api->songdata->tracks);
    return NULL;
}

void *event_track_prev(void *data) {
    api_t *api = (api_t *)data;
    list_active_prev(api->songdata->tracks);
    return NULL;
}
...
void editor_update_normal(api_t *api) {
    switch(api->key) {
        ...
        case TK_TRACK_NEXT:  add_event(api->events, event_track_next, api); break;
        case TK_TRACK_PREV:  add_event(api->events, event_track_prev, api); break;
